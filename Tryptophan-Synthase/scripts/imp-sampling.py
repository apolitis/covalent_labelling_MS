...
def create_representation():
    ...
    def create_protein_from_pdbs(name, files):
        def create_from_pdb(file):
            ...
            datadir = '/u/cvr/apolitis/docs-imp/CovalentLabelling/tryptophanSynth18Sept/'
...
#            return c
    ...
    create_protein_from_pdbs("A1", ["Alpha1.pdb"])
    create_protein_from_pdbs("A2", ["Alpha2.pdb"])
    create_protein_from_pdbs("B1", ["Beta1.pdb"])
    create_protein_from_pdbs("B2", ["Beta2.pdb"])
    return (m, all)
...
def create_restraints(m, all):
    def add_connectivity_restraint(s):
        ...
        iB = r.add_type([rps[2],rps[3]])
        #iC = r.add_type([rps[2]])
        n1 = r.add_composite([iA, iA, iB, iB])
        n2 = r.add_composite([iA, iB, iB], n1)
        n3 = r.add_composite([iB, iB], n2)
        n4 = r.add_composite([iA], n2)
    ...
    sA1=S(hierarchy=all, molecule="A1")
    sA2=S(hierarchy=all, molecule="A2")
    sB1=S(hierarchy=all, molecule="B1")
    sB2=S(hierarchy=all, molecule="B2")
    add_connectivity_restraint([sA1, sA2, sB1, sB2])
...
def get_conformations(m):
    ...
    sampler.set_number_of_monte_carlo_steps(25)
...
f1 = open("MCsampling-Score.csv", "w")
...
for i in range(0, cs.get_number_of_configurations()):
    cs.load_configuration(i)
    w= IMP.display.PymolWriter("native.%d.pym"%i)
    for g in gs:
        w.add_geometry(g)
